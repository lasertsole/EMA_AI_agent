...
class RoutingModelResult(BaseModel):
    ...
    needs_l1: Optional[bool] = Field(
        description="Whether to load L1 layer index (historical key decisions). Set to true when user's question references previous work or requires context from past conversations")
    l1_dates: Optional[List[str]] = Field(
        description="List of dates for L1 decisions to load, Empty array means no specific L1 dates needed",
        examples=[[], ["2026-03-14"], ["2026-03-14", "2026-03-11"]])
    l1_tsids: Optional[List[str]] = Field(
        description="List of tsids for L1 decisions to load, Empty array means no specific L1 tsids needed",
        examples=[[], ["20260309232555"], ["20260309232745", "20260309232555"]])
    needs_l2: Optional[bool] = Field(
        description="Whether to load L2 layer full conversation history. Set to true when complete conversation context is required")


routing_model: Runnable = simple_chat_model.with_structured_output(RoutingModelResult)
...
def build_routing_prompt(user_input: str, file_names: List[str], skills: List[SkillIndexEntry],
                         timeline: Optional[str] = None) -> Prompt:
...
def viking_route(
    ...
    routing_prompt = build_routing_prompt(user_input=user_input, file_names=file_names, skills=skills,
                                          timeline=timeline)
    result: RoutingModelResult | None = call_routing_model(system=routing_prompt["system"], user=routing_prompt["user"])
    ...
    needs_l1: bool = getattr(result, "needs_l1") if getattr(result, "needs_l1", None) is not None else False
    l1_dates: List[str] = sorted(getattr(result, "l1_dates"), reverse=True) if getattr(result, "l1_dates", None) is not None else []
    l1_tsids: List[str] = sorted(getattr(result, "l1_tsids"), reverse=True) if getattr(result, "l1_tsids", None) is not None else []
    needs_l2: bool = getattr(result, "needs_l2") if getattr(result, "needs_l2", None) is not None else False
