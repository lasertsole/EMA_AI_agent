...
"exit": 结束运行
'''
def control_model(command: str = None)-> Response:
...
### 切换GPT模型
def change_GPT_model(weights_path: str = None)-> Response:
...
### 切换sovits模型
def change_sovits_model(weights_path: str = None)-> Response:
    if (weights_path is None):
        return
...
### 切换参考音频
def change_refer_audio(refer_audio_path: str = None):
