...
from ..DAO import enqueue_append_timeline_entry, compress_history
        ...
        if is_stream:
            ...
            async for chunk in agent.astream(messages_dict, config = get_config(session_id), stream_mode = "messages"):
                ...
                if isinstance(msg_chunk, AIMessageChunk):
                    # 以下是输出工具信息
                    tool_calls:List[ToolCall] | List[ToolCallChunk] = msg_chunk.tool_calls if msg_chunk.tool_calls and len(msg_chunk.tool_calls) > 0 else msg_chunk.tool_call_chunks
