...
- ▶️ _详见 [Subagent 系统文档](subagent/README.zh.md)_
...
### 5. ⏰ 定时与主动行为
- **定时任务服务** ([cron/](cron/README.zh.md))：支持一次性、固定间隔和 Cron 表达式三种调度类型的 Agent 任务执行
- **心跳服务** ([heartbeat/](heartbeat/README.zh.md))：定期唤醒 Agent，检查 HEARTBEAT.md 中待处理的任务并在空闲时自动执行
---
...
│   ├── middlewares/        # SummarizationMiddleware、记忆注入
...
│
├── models/                 # 模型封装
│   ├── chat/               # 聊天模型 (LangChain BaseChatModel)
│   ├── reasoner/           # 推理模型 (思维链)
│   ├── vl/                 # 视觉语言模型
│   └── tts/                # TTS 语音合成 (GPT-SoVITS)
│
├── pub_func/               # 公共工具函数
├── rag/                    # 检索增强生成模块
...
| **Subagent System** | 层次化任务分解与并行执行 | [中文](subagent/README.zh.md) · [EN](subagent/README.md) |
| **Cron Service** | 定时/周期 Agent 任务执行 | [中文](cron/README.zh.md) · [EN](cron/README.md) |
| **Heartbeat Service** | 定时唤醒任务检查 | [中文](heartbeat/README.zh.md) · [EN](heartbeat/README.md) |
