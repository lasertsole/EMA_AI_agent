...
### 4. 🔊 多模态交互
- **TTS 语音合成**：基于 CosyVoice 的语音合成，支持高度还原角色声线的实时语音回复（[models/TTS_model/](models/TTS_model/)）
...
│   ├── middlewares/        # 中间件：ContextEngineHook, Summarization,
...
| **MesMemory** | 短期会话消息记忆 | [中文](context_engine/mes_memory/README.zh.md) · [英文](context_engine/mes_memory/README.md) |
...
```bash
cp .env.example .env
# 编辑 .env 配置 MAIN_LLM_API_KEY、TTS 模型路径等
```

### 4️⃣ 启动服务
使用 `start.sh` 一键启动 TTS、本地 Ollama 模型、后端服务和前端 UI。
