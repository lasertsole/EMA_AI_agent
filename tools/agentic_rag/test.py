...
async def main():
    ...
    async def retrieve_callback(query: str) -> List[Document]:
        return await retriever.ainvoke(query, k=10, score_threshold=0.5)
    from query_transformations import generate_queries_decomposition, build_answer_individually_graph
    questions = await generate_queries_decomposition.ainvoke({"question":question})

    answer_individually_graph = build_answer_individually_graph(retrieve_callback)
    print(await answer_individually_graph.ainvoke({"input":{"question": question, "sub_questions": questions}}))
